...
Use menu option `12) Manage email domains/mailboxes` to onboard a new domain in one flow (`add-domain`), change the mail host (`change-mail-host`), create mailboxes, reset mailbox passwords, manage aliases/forwards, manage DKIM from the server, audit DNS records, and repair the Postfix postscreen cache when needed.
...
Menu option `12) Manage email domains/mailboxes` can also manage mail aliases and forwarding:

- `add-alias` - create an address that forwards to one or more recipients, for example `sales@example.com -> user@example.com`
- `delete-alias` - remove one recipient from an alias, or remove all recipients for that alias
- `list-aliases` - show aliases stored in `postfix-virtual.cf`
- `set-forward` - forward an existing mailbox to one or more recipients using a managed Sieve script
- `clear-forward` - remove the managed Sieve forward for a mailbox
- `list-forwards` - show managed mailbox forwards
- `enable-webmail-forwards` - enable docker-mailserver ManageSieve and Roundcube's Filters/Forwarding UI
- `forward-status` - show alias, Sieve, and Roundcube forwarding status

Use an alias when the source address is not a real mailbox. Use mailbox forwarding when the source address already exists as a mailbox. Mailbox forwarding can keep a local copy or redirect without keeping a local copy.

ManageSieve is enabled only on the internal Docker network for Roundcube. The script does not publish port `4190` to the Internet.
...
- Optionally manage mailbox forwards and filters through Roundcube when menu option `12` -> `enable-webmail-forwards` is enabled
